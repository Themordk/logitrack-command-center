...
-- 1. Enums
CREATE TYPE public.enum_tipo_abastecimento AS ENUM ('PREVENTIVO', 'CORRETIVO');
CREATE TYPE public.enum_status_abastecimento AS ENUM ('GERADO', 'EM_EXECUCAO', 'FINALIZADO', 'CANCELADO');
...
CREATE TABLE public.abastecimento (
  ...
  tipo public.enum_tipo_abastecimento NOT NULL,
  status public.enum_status_abastecimento NOT NULL DEFAULT 'GERADO',
...
CREATE POLICY "abastecimento_tenant_policy"
  ON public.abastecimento FOR ALL
  USING (tenant_id = get_current_tenant())
  WITH CHECK (tenant_id = get_current_tenant());

-- 3. Coluna abastecimento_id na tarefa
ALTER TABLE public.tarefa
  ADD COLUMN abastecimento_id uuid REFERENCES public.abastecimento(id);

-- 4. Indices de performance
CREATE INDEX idx_tarefa_abastecimento ON public.tarefa (abastecimento_id) WHERE abastecimento_id IS NOT NULL;
CREATE INDEX idx_tarefa_tipo_status ON public.tarefa (tipo_tarefa_id, status, produto_id);
CREATE INDEX idx_estoque_endereco_produto ON public.estoque_geral (endereco_id, produto_id);
CREATE INDEX idx_picking_produto_armazem ON public.picking_produto (armazem_id, produto_id) WHERE ativo = true;

-- 5. View consolidada
CREATE OR REPLACE VIEW public.vw_abastecimento_lista AS
SELECT
  a.id,
  a.tenant_id,
  a.empresa_id,
...
LEFT JOIN public.armazem az ON az.id = a.armazem_id
LEFT JOIN public.usuario u ON u.id = a.criado_por
LEFT JOIN LATERAL (
  SELECT
    COUNT(*)::integer AS count_tarefas,
    COUNT(*) FILTER (WHERE t.status = 'CONCLUIDA')::integer AS tarefas_concluidas
  FROM public.tarefa t
  WHERE t.abastecimento_id = a.id
) tc ON true;

-- 6. RPC fn_gerar_abastecimento
...
BEGIN
  ...
  IF v_tipo = 'PREVENTIVO' THEN
    -- Preventivo: demanda SEP pendente vs saldo picking
    FOR rec IN
      WITH demanda_sep AS (
        SELECT t.produto_id,
        ...
        IF NOT EXISTS (
          ...
          IF NOT p_simular THEN
            INSERT INTO tarefa (tenant_id, empresa_id, tipo_tarefa_id, produto_id,
              id_local_origem, id_local_destino, quantidade_requerida, armazem_id, abastecimento_id, prioridade)
  ...
  ELSIF v_tipo = 'CORRETIVO' THEN
    -- Corretivo: nivel minimo/maximo picking_produto
    FOR rec IN
