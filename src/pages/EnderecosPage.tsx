...
import { Search, Plus, SlidersHorizontal, ChevronUp, ChevronDown, Eye, Edit2, Trash2 } from "lucide-react";
...
export function EnderecosPage({ onNavigate }: EnderecosPageProps) {
  ...
  const filtered = mockEnderecos
    .filter((e) => {
      const matchSearch = e.codigo.toLowerCase().includes(search.toLowerCase()) || e.setor.toLowerCase().includes(search.toLowerCase());
        ...
        <table className="w-full">
          ...
          <tbody>
            {paginated.map((end, idx) => (
              <tr key={end.id} className={cn("border-b border-border/50 table-row-hover", idx % 2 === 0 ? "" : "bg-secondary/10")}>
                <td className="px-4 py-3 font-mono text-sm font-medium text-foreground">{end.codigo}</td>
                <td className="px-4 py-3"><StatusBadge status={end.tipoEndereco} type="endereco-tipo" /></td>
                <td className="px-4 py-3"><StatusBadge status={end.situacao} type="endereco-situacao" /></td>
                <td className="px-4 py-3">
                  <span className={cn(
                    "inline-flex items-center justify-center w-6 h-6 rounded text-xs font-bold",
                    end.curva === "A" ? "bg-green-500/15 text-green-400" :
                    end.curva === "B" ? "bg-blue-500/15 text-blue-400" :
                    end.curva === "C" ? "bg-yellow-500/15 text-yellow-400" :
                    "bg-slate-500/15 text-slate-400"
                  )}>
                    {end.curva}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm text-muted-foreground">{end.capacidadeM3.toFixed(1)}</td>
                <td className="px-4 py-3 text-sm text-muted-foreground">{end.pesoMaxKg.toLocaleString()}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    {[
                      { icon: <Eye size={13} />, label: "Ver" },
                      { icon: <Edit2 size={13} />, label: "Editar" },
                      { icon: <Trash2 size={13} />, label: "Excluir" },
                    ].map(({ icon, label }) => (
                      <button
                        key={label}
                        className="flex items-center justify-center w-7 h-7 rounded hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors"
                        title={label}
                      >
                        {icon}
                      </button>
