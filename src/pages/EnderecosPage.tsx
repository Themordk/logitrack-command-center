...
export function EnderecosPage({ onNavigate }: { onNavigate?: (path: string) => void }) {
  ...
  const fields: FieldSpec[] = [
    ...
    { name: "tipo_estrutura", label: "Tipo Estrutura", type: "enum", enumValues: ["porta_palete", "blocado", "prateleira", "flow_rack", "drive_in", "mezanino", "doca"] },
    { name: "situacao", label: "Situação", type: "enum", required: true, enumValues: ["LIVRE", "OCUPADO", "BLOQUEADO"] },
    { name: "total_pallet", label: "Total Pallets", type: "number", required: true, placeholder: "2" },
