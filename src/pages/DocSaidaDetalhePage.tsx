...
interface ItemRow {
  ...
  lotes?: { id: string; lote: string; validade: string; fabricacao: string; serie: string; quantidade: number }[];
}

const STATUS_MAP: Record<number, { label: string; cls: string }> = {
  0: { label: "Pendente", cls: "bg-red-500/15 text-red-400 border-red-500/30" },
  1: { label: "Em Onda", cls: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30" },
...
export function DocSaidaDetalhePage({ documentoId, onBack }: Props) {
    ...
    async function load() {
      try {
        ...
        const { data: doc, error: docErr } = await (supabase as any)
          ...
          .select("id, numero_pedido, data_emissao, status, valor_pedido, observacao, vendedor, transportador, parceiro_id, tipo_pedido_id, rota_id, excluido_em, excluido_por, motivo_exclusao_id, observacao_exclusao")
          .eq("id", documentoId)
          .single();
        ...
        const [parceiroRes, tipoRes, rotaRes, itemsRes] = await Promise.all([
          (supabase as any).from("parceiro").select("razaosocial, cnpj").eq("id", doc.parceiro_id).single(),
          doc.tipo_pedido_id
            ? (supabase as any).from("tipo_saida").select("descricao").eq("id", doc.tipo_pedido_id).single()
            : Promise.resolve({ data: null }),
          doc.rota_id
            ? (supabase as any).from("rotas").select("descricao").eq("id", doc.rota_id).single()
            : Promise.resolve({ data: null }),
          (supabase as any)
            .from("documento_saida_item")
            .select("id, produto_id, quantidade, valor_unit, valor_total")
            .eq("documento_saida_id", documentoId),
        ]);

        setParceiro(parceiroRes.data || null);
        setTipoPedido(tipoRes.data?.descricao || "");
        setRota(rotaRes.data?.descricao || "");
        ...
        const [prodRes, lotesRes] = await Promise.all([
          ...
          (supabase as any)
            .from("documento_saida_item_lote")
            .select("id, documento_saida_item_id, lote, validade, fabricacao, serie, quantidade")
            .in("documento_saida_item_id", itemIds),
        ]);
  ...
  return (
    <div className="flex flex-col gap-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-base font-semibold text-foreground">Detalhe do Documento de Saída</h2>
          <p className="text-xs text-muted-foreground">
            Nº Pedido: <span className="font-mono">{header.numero_pedido}</span> · {parceiro?.razaosocial || "—"}
          </p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <InfoItem label="Rota" value={rota} icon={<MapPin size={10} />} />
          <InfoItem label="Vendedor" value={header.vendedor} />
          <InfoItem label="Transportador" value={header.transportador} icon={<Truck size={10} />} />
          <div className="col-span-2 md:col-span-4">
            <InfoItem label="Observação" value={header.observacao} />
          </div>
        </CardContent>
      </Card>
            ...
            <table className="w-full text-sm">
                ...
                {items.map((it) => (
                  <Fragment key={it.id}>
                    <tr className="border-b border-border/50">
                      ...
                      <td className="px-4 py-2.5 text-right font-mono text-muted-foreground">{fmtMoney(it.valor_unit)}</td>
              ...
              <tfoot>
                <tr className="border-t border-border bg-secondary/30 font-medium">
                  ...
                  <td></td>
                  <td className="px-4 py-2.5 text-right font-mono text-foreground">{fmtMoney(header.valor_pedido)}</td>
                </tr>
              </tfoot>
            </table>
          )}
