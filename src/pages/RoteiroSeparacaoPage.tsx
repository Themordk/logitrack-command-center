...
const AGRUPAMENTO_OPTIONS = [
  ...
  { value: "CAPACIDADE_HU", label: "Capacidade HU" },
...
export function RoteiroSeparacaoPage() {
  ...
  // Drag state
  const [dragType, setDragType] = useState<"agrup" | "ordem" | null>(null);
  ...
  useEffect(() => { fetchAgrupamentos(); fetchOrdens(); }, [fetchAgrupamentos, fetchOrdens]);
  ...
  const addAgrupamento = async () => {
    ...
    const { error } = await (supabase as any).from("agrupamento_separacao").insert({
      ...
      tipo_agrupamento: agrupForm.tipo_agrupamento,
      sequencia: seq,
    });
  ...
  const removeAgrupamento = async (id: number) => {
  ...
  const addOrdem = async () => {
    ...
    const { error } = await (supabase as any).from("ordem_expedicao").insert({
      tenant_id: tenantId,
      empresa_id: empresaId,
      rua: Number(ordemForm.rua),
      ordem: ordemForm.ordem,
      sequencia: seq,
    });
    if (error) { toast.error(error.message); return; }
  ...
  const removeOrdem = async (id: number) => {
  ...
  // Drag & Drop handlers
  const handleDragStart = (type: "agrup" | "ordem", idx: number) => {
    setDragType(type);
    setDragIdx(idx);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-lg font-bold text-foreground">Roteiro de Separação</h1>
        <p className="text-xs text-muted-foreground">Configure agrupamento e ordem de separação. Arraste para reordenar.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-surface overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-border">
            <h2 className="text-sm font-semibold text-foreground uppercase tracking-wide">Agrupamento de Separação</h2>
            <button
              onClick={() => { setAgrupForm({ tipo_agrupamento: "" }); setShowAgrupModal(true); }}
          ...
          {loadingAgrup ? (
            ...
            <div className="divide-y divide-border/50">
              {agrupamentos.map((item, idx) => (
                <div
                  key={item.id}
                  draggable
                  onDragStart={() => handleDragStart("agrup", idx)}
                  onDragOver={handleDragOver}
                  onDrop={() => handleDropAgrup(idx)}
                ...
                >
                  ...
                  <span className="text-sm text-foreground flex-1">
                    {AGRUPAMENTO_OPTIONS.find((o) => o.value === item.tipo_agrupamento)?.label || item.tipo_agrupamento}
                  </span>
                  <button onClick={() => removeAgrupamento(item.id!)} className="p-1 rounded hover:bg-destructive/10 transition-colors">
      ...
      <Dialog open={showAgrupModal} onOpenChange={setShowAgrupModal}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>Adicionar Agrupamento</DialogTitle></DialogHeader>
          <div className="py-2">
            ...
            <select value={agrupForm.tipo_agrupamento} onChange={(e) => setAgrupForm({ tipo_agrupamento: e.target.value })} className={inputClass}>
              ...
              {AGRUPAMENTO_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <DialogFooter>
