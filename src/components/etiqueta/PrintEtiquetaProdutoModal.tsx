...
export function PrintEtiquetaProdutoModal({ open, onClose, items }: Props) {
  ...
  items.forEach((it) => {
    const data: LabelData = { barcodeValue: String(it.ean || ""), displayText: it.descricao || "" };
    const r = validateLabel(data, template);
    if (!r.valid) validationErrors.push(`${it.sku || it.produto_id}: ${r.errors.join(", ")}`);
  });
  ...
  const Check = ({ label, k }: { label: string; k: keyof EtiquetaProdutoOptions }) => (
    <label className="flex items-center gap-2 cursor-pointer select-none">
      <input
        type="checkbox"
        checked={!!opt[k]}
        onChange={(e) => setOpt((p) => ({ ...p, [k]: e.target.checked }))}
        ...
        <DialogContent className="max-w-md bg-card border-border max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-foreground">
              ...
              Impressão de Etiquetas de Produto – Padrão CORE
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="border border-border/50 rounded-lg p-3 space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground mb-2">EAN e Fator são impressos por padrão.</p>
              <div className="grid grid-cols-2 gap-y-2">
                <Check label="Incluir QR Code" k="incluirQRCode" />
                <Check label="Incluir Marca" k="incluirMarca" />
                <Check label="Incluir Altura" k="incluirAltura" />
                <Check label="Incluir Largura" k="incluirLargura" />
                <Check label="Incluir Comprimento" k="incluirComprimento" />
                <Check label="Incluir Peso Bruto" k="incluirPesoBruto" />
                <Check label="Incluir Peso Líquido" k="incluirPesoLiquido" />
                <Check label="Incluir M³" k="incluirM3" />
              </div>
            </div>
