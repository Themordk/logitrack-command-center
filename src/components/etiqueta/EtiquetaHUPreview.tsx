  ...
  useEffect(() => {
    ...
    try {
      JsBarcode(svgRef.current, value, {
        format: "CODE128", width: 3, height: 120, margin: 16,
...
function EtiquetaHUSingle({ hu, isPrint, config }: { hu: HULike; isPrint: boolean; config?: EtiquetaTemplateOverride }) {
  ...
  return (
    <div className="etiqueta-thermal" style={{
      ...
      {showHeader && (
        <div style={{ height: "56px", background: "#FFFFFF", borderBottom: "3px solid #000000", display: "flex", alignItems: "center", justifyContent: "space-between", paddingLeft: "16px", paddingRight: "16px", flexShrink: 0 }}>
          {showLogo && config?.logo_url ? (
            <img src={config.logo_url} alt="Logo" style={{ maxHeight: 40, maxWidth: 140, objectFit: "contain" }} />
          ) : (
            <span style={{ color: "#000000", fontSize: `${fs(14)}px`, fontWeight: 900, letterSpacing: "2px", textTransform: "uppercase" }}>
              {config ? "" : "CORE LOGITRACK"}
            </span>
          )}
          <span style={{ color: "#000000", fontSize: `${fs(11)}px`, fontWeight: 700 }}>HU</span>
        </div>
      )}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "12px 16px", background: "#FFFFFF" }}>
        {label ? <BarcodeHU value={label} /> : <span style={{ color: "#CC0000", fontSize: `${fs(10)}px` }}>Código HU ausente</span>}
      </div>
      <div style={{ height: "36px", textAlign: "center", fontSize: `${fs(20)}px`, fontWeight: 900, letterSpacing: "2px", color: "#000000", textTransform: "uppercase", lineHeight: "36px", flexShrink: 0, background: "#FFFFFF" }}>
        {label}
      </div>
      <div style={{ height: "28px", display: "flex", justifyContent: "center", gap: "32px", fontSize: `${fs(11)}px`, fontWeight: 700, color: "#000000", alignItems: "center", flexShrink: 0, background: "#FFFFFF" }}>
        {campos
          ? campos.filter((c) => c.chave !== "codigo_hu").map((c) => {
              const val = c.chave === "tipo_hu" ? hu.tipo_hu : c.chave === "tamanho" ? hu.tamanho : (hu as any)[c.chave];
              if (val == null || val === "") return null;
              return <span key={c.chave}>{(c.label || c.chave).toUpperCase()}: {String(val)}</span>;
            })
          : (<>
