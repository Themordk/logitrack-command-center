...
interface PrintEtiquetaEnderecoModalProps {
  open: boolean;
  onClose: () => void;
  enderecos: Array<{ id: string | number; codigo?: string; descricao?: string; codigo_endereco?: number | string | null; setor?: string; setor_id?: string; tipo_endereco?: string; curva_acesso?: string }>;
}
...
export function PrintEtiquetaEnderecoModal({ open, onClose, enderecos }: PrintEtiquetaEnderecoModalProps) {
  ...
  const triggerPrint = () => {
    ...
    if (!win) return;
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Etiquetas CORE LogiTrack</title><style>*{margin:0;padding:0;box-sizing:border-box;}@page{size:${tamanho === "100x40" ? (orientacao === "horizontal" ? "100mm 40mm" : "40mm 100mm") : (orientacao === "horizontal" ? "50mm 20mm" : "20mm 50mm")};margin:0;}body{background:white;font-family:Arial,Helvetica,sans-serif;}.etiqueta-core{page-break-after:always;break-after:page;}</style></head><body>${printContent.innerHTML}</body></html>`;
      ...
      <Dialog open={open && !showPreview} onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="max-w-md bg-card border-border">
          ...
          <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-lg px-3 py-2">
            ...
            <span className="text-xs text-primary font-medium">{enderecos.length} {plural ? "etiquetas" : "etiqueta"} selecionada{plural ? "s" : ""}</span>
          </div>
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{tamanho} · {orientacao === "horizontal" ? "Horizontal" : "Vertical"} · Padrão CORE</p>
              </div>
            </div>
