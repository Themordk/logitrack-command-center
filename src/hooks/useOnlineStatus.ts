...
async function pingNetwork(): Promise<boolean> {
  ...
  try {
    // Faz fetch do próprio favicon do app com cache-busting.
    // Prova que a rede está funcional sem depender de endpoints autenticados.
    const url = `${window.location.origin}/favicon.ico?_t=${Date.now()}`;
    const response = await fetch(url, {
      method: "HEAD",
      cache: "no-store",
      signal: controller.signal,
    });
    // Qualquer resposta (200, 304) = temos rede
