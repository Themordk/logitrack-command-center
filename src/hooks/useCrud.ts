...
export function useCrud<T extends Record<string, any>>({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (search) {
        query = query.or(buildSearchOrParts(table, search, searchFields).join(","));
      }
  ...
  }, [table, tenantId, empresaId, armazemId, empresaVersion, page, pageSize, search, orderBy, orderDir, select, JSON.stringify(searchFields), JSON.stringify(filters), requiresArmazem, requiresEmpresa]);
  ...
  }, [table, safeTenantId, empresaId, armazemId, search, JSON.stringify(searchFields), JSON.stringify(filters), requiresArmazem, requiresEmpresa]);
